...
from layout_data.Models.model import UNetUnsupLearn, UNetSupLearn

def test(hparams):
    seed = hparams.seed #Sets a seed to randomize the testing from input values for a better loss
    np.random.seed(seed)
    torch.manual_seed(seed)
    cudnn.benchmark = True

    model = UNetUnsupLearn(hparams).cuda() #Tells the testing software to use GPUs by referencing torch cuda.

    model_path = os.path.join(rootPath, 'lightning_logs/version_0/checkpoints/epoch=29-step=239999.ckpt') #Grab the saved model path (must change path if changing reference model)
    model.load_state_dict(torch.load(model_path)['state_dict']) #Load in the model state
    model.prepare_data() 
    data_loader = model.test_dataloader() 
    mae_list, cmae_list, maxae_list, mtae_list, loss_list = [], [], [], [], [] #Initialize losses
    jacobi = Jacobi_layer(nx=200, length=0.1, bcs=[[[0.045, 0.0], [0.055, 0.0]]])
    for idx, data in enumerate(data_loader):
        model.eval()
        layout, heat = data[0].cuda(), data[1].cuda()
        with torch.no_grad():
            heat_pre = model(layout) #Make a heat prediction
